...
export default function ProviderConfigPage({ params }: { params: Promise<{ provider: string }> }) {
  ...
  return <div className="mx-auto grid w-full max-w-5xl gap-5"><AdminPageHeader tag="Envíos" title={provider.name} description="Configurá origen, modalidades y costos manuales." backLink={{ href: "/admin/envios/medios-de-envio", label: "Volver", onNavigate: interceptNavigation }} /><ProviderConfigForm provider={provider} onSubmit={handleSubmit} onDirtyChange={setFormDirty} interceptNavigation={interceptNavigation} />{discardModal}</div>;
}
