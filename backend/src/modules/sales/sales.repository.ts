...
export class SalesRepository {
  ...
  async createManualSale(transaction: TransactionClient, input: CreateSaleRecord): Promise<SalesOrderRecord> {
    return transaction.order.create({
      data: {
        currency: input.currency, customerDni: input.customerDni ?? null, customerEmail: input.customerEmail, customerFirstName: input.customerFirstName, customerLastName: input.customerLastName, customerPhone: input.customerPhone ?? null,
...
export function manualSaleRecord(input: CreateManualSale, number: string): CreateSaleRecord {
  return {
    currency: input.currency, customerDni: input.customer.dni, customerEmail: input.customer.email, customerFirstName: input.customer.firstName, customerLastName: input.customer.lastName, customerPhone: input.customer.phone, customerSnapshot: inputJson(input.customer), deliverySnapshot: inputJson(input.deliverySnapshot), deliveryType: input.deliveryType,
