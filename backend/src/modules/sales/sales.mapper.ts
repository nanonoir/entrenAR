...
export interface AdminSaleSummaryDto { createdAt: string; currency: string; customer: SaleCustomerDto; deliveryType: OrderDeliveryType; id: string; isArchived: boolean; itemCount: number; number: string; paymentStatus: PaymentStatus | null; shippingStatus: OrderShippingStatus; sourceOrderId?: string; status: OrderStatus; total: number; trackingCode?: string; updatedAt: string; }
...
export function toAdminSaleSummaryDto(order: SalesOrderRecord): AdminSaleSummaryDto {
  return { createdAt: order.createdAt.toISOString(), currency: order.currency, customer: customerSnapshot(order), deliveryType: order.deliveryType, id: order.id, isArchived: order.isArchived, itemCount: order.items.reduce((count, item) => count + item.quantity, 0), number: order.number, paymentStatus: order.payment?.status ?? null, shippingStatus: order.shippingStatus, ...(order.sourceOrderId ? { sourceOrderId: order.sourceOrderId } : {}), status: order.status, total: numberValue(order.total), ...(order.shippingTrackingCode ? { trackingCode: order.shippingTrackingCode } : {}), updatedAt: order.updatedAt.toISOString() };
}
